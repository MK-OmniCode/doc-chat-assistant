 ...
 *
 * The gateway is an OpenAI-compatible endpoint. It keeps the model API key on
 * the server (LOVABLE_API_KEY) so the browser never sees a credential.
 *
 * `X-Lovable-AIG-Run-ID` is a correlation id: we resend one if the platform
 * gave us one, and capture the one the gateway mints so the browser can match
 * a request to gateway usage logs.
 */
